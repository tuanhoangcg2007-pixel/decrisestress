...
# --- 1. HUẤN LUYỆN MÔ HÌNH (GIỮ NGUYÊN PERCEPTRON) ---
@st.cache_resource 
def train_model():
...
# --- GIAO DIỆN WEB ---
st.title("🧠 Dự đoán Stress (Perceptron)")

if model:
    st.sidebar.metric("Độ chính xác mô hình", f"{accuracy:.2%}")
    ...
    # --- TỐI ƯU UX: KIỂM TRA TỔNG THỜI GIAN ---
    ...
    if total_hours > 24:
        ...
        if st.button('Dự đoán kết quả', type='primary', use_container_width=True):
            ...
            # Cảnh báo thêm
            if res == 2 or w5 <= 2:
                st.warning("Lời khuyên: Bạn nên dành thời gian nghỉ ngơi và cân bằng lại lịch sinh hoạt.")
else:
